...
[![LINUX DO][linux-do-badge]](https://linux.do)
...
![Make It Real 中文示例效果](assets/screenshots/make-it-real-zh.png)
...
使用 open `skills` CLI 直接安装：

```bash
npx skills add Geo-ff/make-it-real --skill make-it-real -y
```

如果想先查看 CLI 能识别到哪些 skill：

```bash
npx skills add Geo-ff/make-it-real --list
```

这是面向多种 AI agent 工具的通用安装方式。通过 `skills` CLI 安装公开 skill 后，skills.sh 可以通过匿名安装 telemetry 发现它；skills.sh 页面收录和排行榜刷新可能需要等待。
