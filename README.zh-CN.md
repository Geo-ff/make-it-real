...
可以把整个仓库作为插件包使用，也可以只把 `skills/make-it-real/` 复制到 Codex 的 skills 目录。

### Claude / Claude Code

如果你的 Claude 环境支持插件包，可以使用 `.claude-plugin/` 作为入口。否则，把 `skills/make-it-real/SKILL.md` 和 `skills/make-it-real/references/` 下需要的文件作为项目指令提供给 Claude。
...
使用 `gemini-extension.json`，并通过 `GEMINI.md` 作为上下文入口。
